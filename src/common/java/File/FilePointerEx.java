...
public class FilePointerEx {
    ...
    private FilePointerEx(long offset, long length) {
        ...
        // 填充第一个
        pointerArray[0] = FilePointer.build(beginIdx, beginOffset, (int) (length > (long) FileHelper.MAX_BLOCK_LENGTH ? FileHelper.MAX_BLOCK_LENGTH - beginOffset : length));
        // 有超过1个
        int tSize = blockSize - 1;
        if (blockSize > 1) {
            for (int i = 1; i < tSize; i++) {
                pointerArray[i] = FilePointer.build(beginIdx + i, 0, FileHelper.MAX_BLOCK_LENGTH);
            }
        }
        // 处理最后一个
        if (tSize > 0) {
